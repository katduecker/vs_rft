...
figpth = fullfile(mpth,'pilot','results','plots');
...
ft_defaults;
subjcode = 'b593';
megpth = fullfile(mpth,'pilot','meg',subjcode);
...
for fl = 1:2%length(files)
    cfg = [];
    cfg.dataset  = fullfile(megpth,files{fl});
    % get trial specifics
    cfg.path = cdpth;
...
medrt = median(rt);
trimrt = medrt - 2*std(rt);
numtrim = find(rt < trimrt);
% cut out -1 to mean rt + 1
cfg = [];
cfg.latency = [-2 min(rt)+0.5];
...
save(fullfile(pthout,[subjcode,'_coh60_mtlb_freqw_',num2str(frqwdth),'_ft_no0pad_T60.mat']),'coh60', 'psdmeg60', 'psdmisc60','csd60')
...
colorbar
subplot(212)
imagesc(timevec,foi,squeeze(psdmeg60(soiidx(2),:,:)))
title('PSD MEG SOI')
xlabel('time (s)')
ylabel('frequency (Hz)')
axis xy
colorbar
print(fig, fullfile(figpth,['psd_meg_diode_60T_ft_frqw_',num2str(frqwdth),'_',soi{2},'_nohp_no0pad_T60']),'-dpng')

% coherence & CSD% plot coherence and CSD
fig = figure;
subplot(211)
imagesc(timevec,foi,squeeze(csd60(soiidx(2),:,:)))
title('CSD MEG SOI')
xlabel('time (s)')
ylabel('frequency (Hz)')
axis xy
...
colorbar
subplot(212)
imagesc(timevec,foi,squeeze(coh60(soiidx(2),:,:)))
...
%xlim([-1.5 2])
colorbar
print(fig, fullfile(figpth,['csd_coh_ft_frqw_',num2str(frqwdth),'_',soi{2},'_no_hp_no0pad_T60']),'-dpng')
