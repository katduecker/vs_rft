...
% run TFR for each Target tagging frequency 

% [c] Katharina Duecker
% plots

function c2_tfr_Tfreq(s)
...
%% TFR separately for each Target freq

% split trials
clear TFRall badcomps alltrl* cb d dtprt g idx*
...
% find trigger
trig60T = [trigdef{cell2mat(cellfun(@(x) ~isempty(x),strfind(trigdef(:,2),'6067'),'UniformOutput',false)),1}];

trl60T = ismember([rspinfo.trl{:,1}],trig60T);
...
cfg.keeptrials = 'no';
TFR60T = ft_freqanalysis(cfg,data); % 60 Hz Target
TFR60T.grad = mGrad;
cfg.trials = ~trl60T;
TFR67T = ft_freqanalysis(cfg,data); % 67 Hz Target
TFR67T.grad = mGrad;

% combine grads
cfg = [];
cfg.method = 'sum';
TFR60T = ft_combineplanar(cfg,TFR60T);
TFR67T = ft_combineplanar(cfg,TFR67T);

% contrast
TFRcontr6067 = TFR60T;
TFRcontr6067.powspctrm = TFR60T.powspctrm ./ TFR67T.powspctrm - 1;

save(fullfile(tfrpth,[subjfolds{s},'_TFR_Tfreq.mat']),'TFR60T', 'TFR67T', 'TFRcontr6067')
