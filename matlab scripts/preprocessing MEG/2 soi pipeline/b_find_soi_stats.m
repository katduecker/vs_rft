...
for p = 1:length(meginfo.alltrl_bl)
    trlstruct{p} = [meginfo.alltrl_bl{p}(:,3)-2.5*fs,meginfo.alltrl_bl{p}(:,3)+fs*2,repmat(-2.5*fs,size(meginfo.alltrl_bl{p},1),1)];
...
meginfo.rejtrl_all = [meginfo.rejtrl_all,find(time_end~=2)];
save(fullfile(pth,'results','meg', '2 merged edf mat', subjfolds{s},'trl_overlap_meg_el_rsp.mat'),'meginfo','-append')
...
meginfo.alltrl_list = meginfo.alltrl_list(logical(keep_trl),:);
