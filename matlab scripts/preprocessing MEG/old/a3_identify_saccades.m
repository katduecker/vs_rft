...
mtpth = fullfile(pth,'matlab scripts');
...
addpath(fullfile(mtpth,'cbrewer'))
pltpth = fullfile(pth,'results','eyelink','figures');
mkdir(pltpth)
elpth = fullfile(pth,'results','eyelink','1 identify saccades');
mkdir(elpth)
prepropth = fullfile(pth,'results','meg','2 merged edf mat');
% this is needed to read in the edf file properly
addpath(fullfile('Y:\Visual Search RFT\matlab scripts','edf-converter-master')); %edf2mat converter
...
for s = 14:length(subjfolds)
    ...
    if exist(fullfile(trl_merge_pth, subjfolds{s},'trl_overlap_meg_el_rsp.mat'))
        load(fullfile(trl_merge_pth, subjfolds{s},'trl_overlap_meg_el_rsp.mat'))
        if s == 14
            elinfo.fsample = 1000;
        end
    else
        disp('Rejected subject, moving on..')
        continue
    end
    ...
    if exist(fullfile(dtpth,subjfolds{s},'el_edf2mat.mat'))
        ...
        disp('No converted edf file, moving on..')
        continue
    end
    ...
    eltrl = elinfo.eltrl;
    elxy = elinfo.move;
    eltrl(elinfo.el_rejtrl,:) = [];
    elxy(elinfo.el_rejtrl,:,:) = [];
